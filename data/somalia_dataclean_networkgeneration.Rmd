...
Import dataset
Data cleaning on settlement name column
```{r}
...
create the network edge list where Source is $Origin_District_country$ and Target is $District Name$
```{r}
...
Temporal network generation under 3 period separation
```{r}
...
# general summary of statistics of three networks
...
Community detection and the periodic change under Infomap Algorithem
```{r}
...
Create the network based on settlement level
```{r}
...
Summarise the reasons for displacement by origin district
Pie chart on the district level displacement reason
```{r}
library(ggplot2)
...
```
Pie chart on the region level displacement reason
```{r}
library(dplyr)
...
Component analysis on networks
```{r}
# Basic properties
...
Visulisation of flow on the Somalia map
```{r}
library(png)
library(grid)
library(ggplot2)
